...
#if defined(CFG_eu868) || defined(CFG_as923) || defined(CFG_eu433)
...
static void initJoinLoop (void) {
    ...
    LMIC.adrTxPow = 14;
    setDrJoin(DRCHG_SET, DR_SF8); // Fabien : Change the joining SF
...
bit_t LMIC_setupChannel (u1_t chidx, u4_t freq, u2_t drmap, s1_t band) {
    if( chidx >= MAX_CHANNELS )
        return 0;
    if( band == -1 ) {
        if( freq >= 434400000 && freq <= 434650000 )
            freq |= BAND_DECI;   // 10% 27dBm
        else if( (freq >= 433000000 && freq <= 433600000) ||
                 (freq >= 434700000 && freq <= 435000000)  )
            freq |= BAND_CENTI;  // 1% 14dBm
        else
            freq |= BAND_MILLI;  // 0.1% 14dBm
    } else {
...
static void initJoinLoop (void) {
    LMIC.txChnl = os_getRndU1() % 3;
    LMIC.adrTxPow = 14;
    setDrJoin(DRCHG_SET, DR_SF8);  //original SF is 7
    initDefaultChannels(1);
    ASSERT((LMIC.opmode & OP_NEXTCHNL)==0);
    LMIC.txend = LMIC.bands[BAND_MILLI].avail + rndDelay(8);
}
...
static int decodeBeacon (void) {
    ...
    if(
#if (CFG_eu868 || CFG_as923 || CFG_eu433)
...
#if defined(CFG_eu868) || defined(CFG_as923) || CFG_eu433
